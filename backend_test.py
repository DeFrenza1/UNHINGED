...
class UnhingedAPITester:
    ...
    def test_discovery_and_matching_flow(self):
        ...
        success, profile_a_response = self.run_test("Complete User A Profile", "PUT", "profile", 200, profile_a, headers_a)
        if not success:
            return False
        
        # Debug: Check if profile is marked as complete
        print(f"   Debug: User A profile_complete: {profile_a_response.get('profile_complete')}")
        if not profile_a_response.get('profile_complete'):
            self.log_test("User A Profile Completion", False, "Profile not marked as complete despite having required fields")
            return False
        ...
        profile_b = {
            ...
            "pref_age_min": 22,
            "pref_age_max": 28,
            "pref_genders": ["Woman"],  # Fixed: Use "Woman" to match gender_identity
            "relationship_type": "Long-term",
            "has_kids": "No", 
            "wants_kids": "Yes"
        }
        ...
        success, profile_c_response = self.run_test("Complete User C Profile", "PUT", "profile", 200, profile_c, headers_c)
        if not success:
            return False
        
        # Debug: Check if profile is marked as complete
        print(f"   Debug: User C profile_complete: {profile_c_response.get('profile_complete')}")
        if not profile_c_response.get('profile_complete'):
            self.log_test("User C Profile Completion", False, "Profile not marked as complete despite having required fields")
            return False
